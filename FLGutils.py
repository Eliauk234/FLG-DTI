...
def load_frag_params(datasetSmiles, datasetProtein, datasetLabel):
    ...
    words2idx_d = dict(zip(frag_set_d, range(1, len(frag_set_d) + 1)))
    words2idx_p = dict(zip(frag_set_p, range(1, len(frag_set_p) + 1)))
...
class EarlyStopping:
    def __init__(self, savepath, patience=80, verbose=False, delta=0, metric='Accuracy_dev', mode='max'):
    ...
    def __call__(self, score, model, epoch):
        if self.mode == 'max':
            score_to_compare = -float('inf') if self.best_score is None else self.best_score
            if score > score_to_compare + self.delta:
    ...
    def save_checkpoint(self, score, model, epoch):
        if self.verbose:
            if self.best_score is None:
                print(f'Epoch {epoch}: Validation {self.metric} improved to {score:.6f}.  Saving model ...')
            else:
                print(f'Epoch {epoch}: Validation {self.metric} improved ({self.best_score:.6f} --> {score:.6f}).  Saving model ...')
        torch.save(model.state_dict(), os.path.join(self.savepath, 'valid_best_checkpoint.pth'))
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        x = x.permute(1, 0, 2)  # 调整输入形状为 (seq_len, batch_size, embed_dim)
        attn_output, _ = self.attention(x, x, x)
        return attn_output.permute(1, 0, 2)  # 恢复形状为 (batch_size, seq_len, embed_dim)
